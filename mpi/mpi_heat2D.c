...
#define BLOCK_LENGTH   	1166		/* dimensions of one block (they are rectangles of size: BLOCK_LENGTH x BLOCK_LENGTH */
...
int main ( int argc, char *argv[] )
{
	...
	MPI_Barrier(MPI_COMM_WORLD);
        ...
        for ( it = 1; it <= STEPS; it++ ) {
            ...
            update ( start_x+1,end_x-1,start_y+1,end_y-1,BLOCK_LENGTH+2,&u[iz][0][0],&u[1-iz][0][0] );            
